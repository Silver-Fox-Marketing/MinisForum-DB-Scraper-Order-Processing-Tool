#!/usr/bin/env python3
"""
Clear and re-import VIN logs for specified dealerships
Imports from shared_resources/Vinlogs directory with proper order number formatting
"""
...
DEALERSHIP_MAPPING = {
    'Porsche St. Louis': {
        ...
        'file': 'PORSCHESTL_VINLOG - LOG.csv',
        'name': 'Porsche St. Louis'
    },
    'South County DCJR': {
        'table': 'south_county_dcjr_vin_log',
        'file': 'SOCODCJR_VINLOG - LOG (2).csv',
        'name': 'South County Dodge Chrysler Jeep RAM'
    },
    'Suntrup Ford Westport': {
        'table': 'suntrup_ford_westport_vin_log',
        'file': 'SUNTRUP_FORD_WESTPORT_VINLOG - LOG (1).csv',
        'name': 'Suntrup Ford Westport'
    },
    'Volvo Cars West County': {
        'table': 'volvo_cars_west_county_vin_log',
        'file': 'VOLVO_WC_VINLOG - LOG.csv',
        'name': 'Volvo Cars West County'
    }
}
        ...
        with open(csv_file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                if existing:
                    ...
                    if table_name == 'south_county_dcjr_vin_log':
                        update_query = f"""
                            UPDATE {table_name}
                            SET order_number = %s, created_at = %s, processed_date = %s, order_type = %s
                            WHERE vin = %s
                        """
                        params = [order_number, datetime.now(), datetime.now().date(), 'CAO', vin]
                    else:
                        update_query = f"""
                            UPDATE {table_name}
                    ...
                    # Insert new record - handle different table schemas
                    if table_name == 'south_county_dcjr_vin_log':
                        # This table has additional required fields
                        insert_query = f"""
                            INSERT INTO {table_name} 
                            (vin, order_number, created_at, processed_date, order_type)
                            VALUES (%s, %s, %s, %s, %s)
                        """
                        params = [vin, order_number, datetime.now(), datetime.now().date(), 'CAO']
                    else:
                        insert_query = f"""
                            INSERT INTO {table_name} 
...
def main():
    ...
    print("=== CLEARING AND IMPORTING VIN LOGS ===")
