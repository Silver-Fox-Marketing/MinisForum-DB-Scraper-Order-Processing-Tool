...
class TemplateBuilder {
    ...
    toggleFieldSelection(fieldChip) {
        ...
        const spanElement = fieldChip.querySelector('span');
        const iconElement = fieldChip.querySelector('i');

        if (!spanElement || !iconElement) {
            console.error('[TEMPLATE BUILDER] Field chip missing required elements:', fieldChip);
            return;
        }

        const fieldLabel = spanElement.textContent;
        const fieldIcon = iconElement.className;
