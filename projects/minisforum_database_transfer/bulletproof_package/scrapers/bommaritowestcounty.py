...
class BOMMARITOWESTCOUNTY():
	...
	def processing_each_vehicle(self, json_data):
		...
		# print(json_data['VehicleLocationLabel'].lower())

		if json_data['VehicleLocationLabel'].lower() != 'bommarito west county':
			return True
		...
		street_addr = '15736 Manchester Road'
		locality = 'Ellisville'
		postal_code = '63011'
		region = 'MO'
		country = 'US'
	...
	def start_scraping_bommaritowestcounty(self):
		...
		soup = self.helper.make_soup_url('https://www.bommaritowestcounty.com/searchall.aspx?pt=1')
		...
		while 1:
			...
			url = f'https://www.bommaritowestcounty.com/api/vhcliaa/vehicle-pages/cosmos/srp/vehicles/{dealer_id}/{page_id}?'
			url += f'?Location=Ellisville%2C%20MO&host=www.bommaritowestcounty.com&pt={page_num}&pn=12&baseFilter=dHlwZSA9ICduJw==&displayCardsShown={display_cards}'
