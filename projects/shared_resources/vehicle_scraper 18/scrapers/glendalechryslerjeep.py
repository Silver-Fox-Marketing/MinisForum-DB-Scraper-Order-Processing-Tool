...
class GLENDALECHRYSLERJEEP():
	...
	def processing_each_vehicle(self, json_data, vehicle_url, mode):
		...
		if status == 'live':
			...
			status = 'In Transit'
		elif status in ['in_transit_at_factory']:
			status = 'Being Built'
		else:
			status = ''
		...
		street_addr = '10070 Manchester Rd'
		locality = 'Glendale'
		postal_code = '63122'
		region = 'MO'
		country = 'US'
		location = 'Glendale Chrysler Jeep Dodge Ram'

		# if vehicle_url == 'https://www.glendalechryslerjeep.net/used/Dodge/2021-Dodge-Durango-st-louis-b0557217ac184a0350c1e1c3671d7017.htm':
		# 	input('wait?')
	...
	def start_scraping_glendalechryslerjeep(self):
		...
		for mode in ['used', 'new']:
			...
			while 1:
				...
				if mode == 'used':
					url = f'https://www.glendalechryslerjeep.net/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start={page_num}'
				else:
					url = f'https://www.glendalechryslerjeep.net/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start={page_num}'
				...
				for index, vehicle in enumerate(all_vehicles):
					...
					vehicle_url = f'https://www.glendalechryslerjeep.net' + vehicle['link']
				...
				if len(all_vehicles) < 16 or page_num >= total_vehicles:
					break
