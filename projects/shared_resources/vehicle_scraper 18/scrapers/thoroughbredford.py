...
class THOROUGHBREDFORD():
	...
	def processing_each_vehicle(self, json_data):
		...
		street_addr = '8501 N Boardwalk Avenue'
		locality = 'Kansas City'
		postal_code = '64154'
		region = 'MO'
		country = 'US'
		location = 'Thoroughbred Ford'
	...
	def start_scraping_thoroughbredford(self):
		...
		soup = self.helper.make_soup_url('https://www.thoroughbredford.com/searchall.aspx?pt=1')
		...
		while 1:
			...
			url = f'https://www.thoroughbredford.com/api/vhcliaa/vehicle-pages/cosmos/srp/vehicles/{dealer_id}/{page_id}?pt={page_num}'
			url += f'&Dealership=Thoroughbred%20Ford&host=www.thoroughbredford.com&pn=12&displayCardsShown={display_cards}'
			...
			for index, vehicle in enumerate(all_vehicles):
				...
				# Skip if VehicleCard is None
				if vehicle is None:
					print(f'Skipping vehicle at index {index}: VehicleCard is None')
					continue
