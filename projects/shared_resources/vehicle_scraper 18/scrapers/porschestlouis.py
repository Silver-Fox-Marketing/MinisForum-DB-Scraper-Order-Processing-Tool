    ...
    def get_all_vehicles(self, url):
       ...
       while True:
          try:
             ...
             print('⚠️ error in getting vehicles:', error)
          time.sleep(1)

    def processing_each_vehicle(self, vehicle_url):
       soup = self.get_all_vehicles(vehicle_url)

       # Updated: Use lambda to match any div containing both 'vdp' and 'vdp--mod'
       json_data = soup.find('div', class_=lambda c: c and 'vdp' in c.split() and 'vdp--mod' in c.split())

       if not json_data:
          print(f"❌ ERROR: Could not find vehicle data container for {vehicle_url}")
          return

       try:
          vin = json_data.get('data-vin', '').strip()
          stock = json_data.get('data-stocknum', '').strip()
          v_type = json_data.get('data-vehicletype', '').title()

          if v_type == 'Used' and json_data.get('data-cpo', '').lower() == 'true':
             v_type = 'Certified Pre-Owned'

          year = json_data.get('data-year', '')
          make = json_data.get('data-make', '')
          model = json_data.get('data-model', '')
          trim = json_data.get('data-trim', '')
          ext_color = json_data.get('data-extcolor', '')
          status = 'In Stock' if json_data.get('data-intransit', '').lower() == 'false' else 'In Transit'
          price = json_data.get('data-price', '')
          body = json_data.get('data-bodystyle', '')
          fuel_type = json_data.get('data-dotagging-item-fuel-type', '')
          msrp = json_data.get('data-msrp', '')
          date_in_stock = json_data.get('data-dotagging-item-inventory-date', '')
          ...
          print(f"❌ ERROR extracting vehicle attributes from {vehicle_url}:", e)
          time.sleep(1)
